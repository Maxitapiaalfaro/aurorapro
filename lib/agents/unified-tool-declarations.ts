    ...
    functionDeclarations: [
      // ─── SEARCH TOOLS ─────────────────────────────────────────────
      {
        ...
        description: [
          ...
          'Retorna artículos con excerpts relevantes, DOIs, autores y metadata. Resultados se cachean 5 minutos — queries idénticas retornan instantáneamente.',
          '',
          'USA CUANDO:',
          '- El terapeuta solicita evidencia empírica explícitamente ("¿hay estudios sobre...?")',
          '- Necesitas validar una hipótesis clínica con datos cuantitativos',
          '- Comparas intervenciones terapéuticas y necesitas meta-análisis o RCTs',
          '- Documentas una formulación que requiere respaldo científico citado',
          '- Hay una afirmación empírica cuestionable que requiere verificación',
          '- El terapeuta pide protocolos específicos basados en evidencia',
          '',
          'NO USES CUANDO:',
          '- La consulta es puramente reflexiva y no necesita datos empíricos',
          '- Ya buscaste sobre el mismo tema en esta conversación — REUTILIZA esa evidencia directamente',
          '- El terapeuta pide un juicio clínico experiencial, no datos de investigación',
          '- Es un concepto clínico básico establecido que no requiere búsqueda (ej: "qué es TCC")',
          '- Es follow-up conversacional sobre evidencia ya presentada',
          '- La pregunta se puede responder con conocimiento clínico general',
          '',
      ...
      // ─── SUB-AGENT TOOLS ─────────────────────────────────────────
      {
        ...
        description: [
          'Sub-agente que agrega y sintetiza el contexto clínico completo de un paciente: registro clínico + memorias inter-sesión + patrones semánticos → resumen clínico integrado.',
          'Usa LLM secundario para síntesis comprensiva con selección semántica inteligente de memorias. Duración: ~1-2 segundos.',
          '',
          'USA CUANDO:',
          '- Primera mención de un paciente en la sesión y necesitas panorama completo',
          '- El terapeuta pide "recuérdame este caso" o "qué sabemos de este paciente"',
          '- Vas a iniciar una formulación de caso y necesitas toda la información integrada',
          '- Necesitas más que solo el registro o solo las memorias — necesitas la síntesis contextual',
          '- El terapeuta pregunta por "historial", "contexto" o "antecedentes" del paciente',
          '',
          'NO USES CUANDO:',
          '- Solo necesitas un dato puntual específico (usa get_patient_record o get_patient_memories directamente)',
          '- Ya sintetizaste el contexto de este paciente en este turno — reutiliza la síntesis',
          '- No hay paciente activo en la sesión',
          '- El terapeuta solo quiere confirmar un dato simple (ej: "cuántos años tiene?")',
          '',
          'OPTIMIZACIÓN: Costoso (~1-2s + LLM call). Si ya tienes el contexto en la conversación, úsalo directamente.',
        ].join('\n'),
        parametersJsonSchema: {
          type: 'object',
          properties: {
