...
/**
 * Standalone intent classification (used by routeUserInput fallback path).
 */
export async function classifyIntent(
  ...
  try {
    const contextPrompt = buildContextualPrompt(userInput, sessionContext, enrichedSessionContext, contextWindowManager);
    ...
    const result = await withRetry(() => ai.models.generateContent({
      ...
      config: {
        tools: [{
          functionDeclarations: INTENT_FUNCTION_DECLARATIONS
        }],
        toolConfig: {
          functionCallingConfig: {
            mode: FunctionCallingConfigMode.ANY,
    ...
    const confidence = calculateEnhancedConfidence(functionCall, userInput, result.usageMetadata);
