...
- **2026-04-10:** Academic Research Workflow Optimization — Completed 3-phase fix for polypharmacy query failures. **Phase 2.1 (Loop Detection):** Implemented SHA-256 hash-based duplicate detection in streaming-handler.ts (~220 lines). Request-scoped toolCallHistory Map with 60s window. Triggers pharmacological fallback on 3rd identical attempt. Prevents infinite retry loops (was consuming ~16,500 tokens). **Phase 2.2 (4-Level Fallback):** Added polypharmacy detection (12 psychiatric drugs) + generateFallbackLevels() in research-evidence.ts. Sequential execution: Level 1 (full query, minResults: 3) → Level 2 (pairwise interactions, minResults: 2) → Level 3 (individual mechanisms, minResults: 1) → Level 4 (drug classes). Early exit on success. Progressive trust score relaxation (60→50→40→30). Pharmacological fallback using Gemini Flash-Lite (~400 tokens) when no literature exists. **Phase 2.3 (Tool Declaration):** Updated research_evidence in unified-tool-declarations.ts with PICO format guidance and 4-level strategy documentation. **Token efficiency:** Loop detection overhead ~150 tokens/call, worst-case total <8,000 tokens (vs original ~16,500). All search levels use Parallel AI exclusively (per user request). **Key learning:** Complex polypharmacy queries (e.g., "Venlafaxine + Lisdexamfetamine + Mirtazapine + ASD + Bipolar") now decompose automatically rather than failing with empty results.
